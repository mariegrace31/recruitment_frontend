...
const jobs = [
  { title: "Software Engineer", desc: "Join our tech team to develop innovative solutions. Minimum 3 years of experience required.", link: "/jobdetail" },
  { title: "Caring Nanny Needed", desc: "Seeking a compassionate and dependable Nanny to provide exceptional care and support for children", link: "/job5" },
  { title: "Content Creator", desc: "As a Content Creator, you will be the voice of our brand, responsible for developing and producing high-quality content.", link: "/job9" },
  { title: "Tutor", desc: "As a Tutor, you will provide individualized academic support to students of various ages and levels.", link: "/job4" },
  { title: "Graphic Designer", desc: "As a Graphic Designer, you will be the visual storyteller for our brand, crafting engaging and memorable designs.", link: "/job2" },
  ...
  return (
    <div className='bg-[#F8FAFF]'>
      ...
      <div className='grid grid-cols-1 lg:grid-cols-3 gap-7 p-6 lg:p-24'>
        {jobs.map((job, index) => (
          <div key={index} className='flex flex-col gap-3 border border-secondary p-4 py-8 rounded-lg bg-white hover:bg-[#E0E6F7] group'>
            <h1 className='font-bold text-lg'>{job.title}</h1>
            <div className='flex gap-4 text-gray-400 text-sm'>
              <p className='flex items-center gap-1'><TbBriefcase /> Full Time</p>
              <p className='flex items-center gap-1'><TbBriefcase /> Part Time</p>
            </div>
            <p className='text-sm'>{job.desc}</p>
            <Link href={job.link} className='bg-[#E0E6F7] p-2 rounded-md w-[40%] ml-auto text-[13px] text-secondary group-hover:bg-secondary group-hover:text-white text-center'>
              More Details
            </Link>
          </div>
        ))}
